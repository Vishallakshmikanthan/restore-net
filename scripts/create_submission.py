...
kla_dir = project_root / "kla-image-restoration"
for p in [str(project_root), str(kla_dir)]:
    if p not in sys.path:
        sys.path.insert(0, p)


def resolve_file(rel_path: str) -> Path:
    for candidate in [project_root / rel_path, kla_dir / rel_path]:
        if candidate.exists():
            return candidate
    return None
...
def create_submission():
    ...
    res_import = subprocess.run(import_cmd, cwd=str(kla_dir if kla_dir.exists() else project_root), capture_output=True, text=True)
    ...
    def filter_tar(tarinfo):
        # Exclude data/, logs/, __pycache__, and intermediate checkpoints
        name = tarinfo.name
    ...
    with tarfile.open(archive_path, "w:gz") as tar:
        ...
        for root_item in ["inference.py", "README.md", "requirements.txt", "pyproject.toml", "solution_presentation.pptx"]:
            p = project_root / root_item
            if p.exists():
                tar.add(p, arcname=root_item)
                print(f"  + Added: {root_item}")

        # Add kla-image-restoration folder or src/
        if kla_dir.exists():
            tar.add(kla_dir, arcname="kla-image-restoration", filter=filter_tar)
            print("  + Added: kla-image-restoration/")

        # Ensure checkpoints/best_model.pt is included
